...
### Function: ProtectedID(appID string) string
...
Please do note, that at least `machine-id` and `MachineGuid` can be changed by root/admin, although that may not come without cost (broken system services and more).
